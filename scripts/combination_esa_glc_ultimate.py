...
def generate_true_stratified_points():
    ...
    # ==========================================
# 3. Main function: Ultimate automation (basin folders + year matching + GLC appended extraction)
# ==========================================
if __name__ == "__main__":
    # Set the root directory for data storage
    esa_root_dir = "data/ESA_CCI_LC_reclassified"
    glc_root_dir = "data/glc-fcs30"
    
    # Define the four major basins array
    all_basins = ["crati", "ebro", "po", "tevere"]
    
    print(f"--- 🚀 Starting fully automated ultimate batch pipeline (Four major basins | Folder categorized storage) ---")
    ...
    for current_basin in all_basins:
        ...
        # [New Logic] Create a dedicated folder for the current basin
        # exist_ok=True means it won't throw an error if the folder already exists
        output_folder = f"results_{current_basin}"
        ...
        for year in range(1980, 2027):
            ...
            # 1. Fuzzy match ESA and GLC year files
            try:
                esa_files = [f for f in os.listdir(esa_root_dir) if re.search(year_str, f)]
                glc_files = [f for f in os.listdir(glc_root_dir) if re.search(year_str, f)]
            except FileNotFoundError:
                continue

            # 2. Conditional check: Execute only if both have data for that year
            if esa_files and glc_files:
                print(f"\n  [{current_basin.upper()} | {year_str}] Match successful! Starting dual sampling...")
                
                # Update path global variables
                global reference_raster_path
                reference_raster_path = os.path.join(esa_root_dir, esa_files[0])
                glc_raster_path = os.path.join(glc_root_dir, glc_files[0])
                ...
                # [Modify Path] Append output path to the corresponding folder
                file_name = f"{current_basin}_{year_str}_ESA_AND_GLC_COMPARISON.csv"
                current_output_csv = os.path.join(output_folder, file_name)
                
                # ==========================================
                # Appended task: Extract data from GLC
                # ==========================================
                print(f"    -> Extracting GLC data...")
                
                gdf_for_sampling = gpd.GeoDataFrame(
                    df_points, 
                    geometry=gpd.points_from_xy(df_points.Longitude, df_points.Latitude),
                    crs="EPSG:4326"
                )
                
                with rasterio.open(glc_raster_path) as glc_src:
                    gdf_glc_crs = gdf_for_sampling.to_crs(glc_src.crs)
                    coord_list = [(geom.x, geom.y) for geom in gdf_glc_crs.geometry]
                    glc_codes = [val[0] for val in glc_src.sample(coord_list)]
                    
                df_points['GLC_Class_Code'] = glc_codes
                df_points['GLC_Class_Name'] = df_points['GLC_Class_Code'].map(lambda x: target_mapping.get(x, 'Unknown'))
                
                # Organize final column order
                final_columns = [
                    'Point_ID', 'Longitude', 'Latitude', 
                    'Map_Class_Code', 'Map_Class_Name', 
                    'GLC_Class_Code', 'GLC_Class_Name', 
                    'GT_Class_Code', 'Notes'
                ]
                ...
                print(f"    ✅ Sampling complete and saved to folder: {current_output_csv}")
                
            else:
                continue

    print("\n🎉 Task successfully completed! All files have been categorized by basin and stored in their respective results_xxx folders.")